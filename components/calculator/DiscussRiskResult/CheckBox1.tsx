...
const initialValues = [
  { label: "Smoking cessation", checked: true, key: randomId() },
  { label: "Lower blood pressure", checked: true, key: randomId() },
  { label: "Lower LDL cholesterol", checked: true, key: randomId() },
];
      ...
      <Checkbox
        ...
        onChange={() => {
          handlers.setState((current) => {
            return current.map((value) => ({ ...value, checked: !allChecked }));
          });
        }}
      />
      {props.hideSmoking
        ? items.filter((i) => {
            return i.props.label !== "Smoking cessation";
          })
        : items}
    </Box>
  );
